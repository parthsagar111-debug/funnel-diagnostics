...
1. "order_level" — raw D2C order history (customer_id, order_date, order_value).
   ...
   dependency, channel repeat-rate analysis, and an orders-over-time trend.
...
2. "metrics_snapshot" — any pre-aggregated funnel metric, in long/tidy format
   ...
   and time period. Computes period-over-period deltas, flags anomalies, and
   reshapes readings into per-metric time series for charting.
"""
...
def compute_orders_trend(df: pd.DataFrame) -> dict:
    """Monthly new-customer and repeat-order counts, for the orders-over-time
    chart. A customer's first order month counts toward 'new'; any order in a
    later month from that same customer counts toward 'repeat'."""
    df = df.copy()
    df["order_month"] = df["order_date"].dt.to_period("M")
    first_order_month = df.groupby("customer_id")["order_month"].min()
    df["cohort_month"] = df["customer_id"].map(first_order_month)
    ...
    new_customers = [int((first_order_month == m).sum()) for m in months]
    repeat_orders = [
        int(((df["order_month"] == m) & (df["cohort_month"] != m)).sum())
        for m in months
    ]
...
def compute_metric_series(df: pd.DataFrame) -> dict:
    """Reshape into {metric_name: {dates: [...], segments: {segment: [values]}}}
    for small-multiple trend charts — one mini chart per metric, segments
    overlaid as separate lines."""
    result = {}
    for metric in sorted(df["metric_name"].unique()):
        mdf = df[df["metric_name"] == metric]
        dates = sorted(mdf["date"].unique())
        date_strs = [pd.Timestamp(d).strftime("%Y-%m-%d") for d in dates]
        segments = {}
        for segment in sorted(mdf["segment"].unique()):
            sdf = mdf[mdf["segment"] == segment].set_index("date")
            segments[segment] = [
                round(float(sdf.loc[d, "value"]), 4) if d in sdf.index else None
                for d in dates
            ]
        result[metric] = {"dates": date_strs, "segments": segments}
    return result
